 ...
 */
package sssc.processors.customlists3;
...
@Tags({"Amazon", "S3", "AWS", "list", "custom"})
@CapabilityDescription("Retrieves a listing of objects from an S3 bucket based on incoming flowfiles. "
        + "For each object that is listed, creates a FlowFile that represents the object. "
        + "Attributes from the incoming FlowFile will be copied to the output FlowFiles. "
        + "The incoming FlowFile is dropped after processing.")
@Stateful(scopes = Scope.CLUSTER, description = "After performing a listing of keys, the timestamp of the newest key is stored, "
        ...
        + "this date the next time that the Processor is run.")
@WritesAttributes({
...
public class CustomListS3 extends AbstractS3Processor {
    ...
    public static final AllowableValue NO_TRACKING = new AllowableValue("none", "No Tracking",
            "This strategy lists all entities without any tracking. The same entities will be listed each time" +
                    " this processor is scheduled.");

    public static final PropertyDescriptor LISTING_STRATEGY = new Builder()
        .name("listing-strategy")
        .displayName("Listing Strategy")
        .description("Specify how to determine new/updated entities.")
        .required(true)
        .allowableValues(BY_TIMESTAMPS, NO_TRACKING)
    ...
    public static final PropertyDescriptor DELIMITER = new Builder()
            ...
            .description("The string used to delimit directories within the bucket.")
            .build();
    ...
    public static final PropertyDescriptor PREFIX = new Builder()
            ...
            .required(false)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .description("The prefix used to filter the object list.")
            .build();
    ...
    public static final PropertyDescriptor USE_VERSIONS = new Builder()
            ...
            .defaultValue("false")
            .description("Specifies whether to use S3 versions, if applicable.")
            .build();
    ...
    public static final PropertyDescriptor MIN_AGE = new Builder()
            ...
            .description("The minimum age that an S3 object must be in order to be considered")
            .required(true)
    ...
    public static final PropertyDescriptor MAX_AGE = new Builder()
            ...
            .description("The maximum age that an S3 object can be in order to be considered")
            .required(false)
            .addValidator(StandardValidators.TIME_PERIOD_VALIDATOR)
    ...
    public static final PropertyDescriptor REQUESTER_PAYS = new Builder()
            ...
            .required(true)
            .description("If true, indicates that the requester consents to pay any charges associated with listing the S3 bucket.")
    ...
    public static final PropertyDescriptor RECORD_WRITER = new Builder()
        ...
        .description("Specifies the Record Writer to use for creating the listing.")
        .required(false)
    ...
    static final PropertyDescriptor BATCH_SIZE = new Builder()
        ...
        .description("If not using a Record Writer, this property dictates how many S3 objects should be listed in a single batch.")
        .required(false)
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        final Map<String, String> incomingAttributes = incomingFlowFile.getAttributes();
        session.remove(incomingFlowFile);

        final String listingStrategy = context.getProperty(LISTING_STRATEGY).getValue();

        if (BY_TIMESTAMPS.equals(listingStrategy)) {
            listByTrackingTimestamps(context, session, incomingAttributes);
        } else if (NO_TRACKING.equals(listingStrategy)) {
            listNoTracking(context, session, incomingAttributes);
        } else {
            throw new ProcessException("Unknown listing strategy: " + listingStrategy);
        }
    }

    private void listNoTracking(ProcessContext context, ProcessSession session, Map<String, String> incomingAttributes) {
        final AmazonS3 client = getS3Client(context, incomingAttributes);

        S3BucketLister bucketLister = getS3BucketLister(context, client, incomingAttributes);
        ...
        final String bucket = context.getProperty(BUCKET_WITHOUT_DEFAULT_VALUE).evaluateAttributeExpressions(incomingAttributes).getValue();
        ...
        if (writerFactory == null) {
            writer = new AttributeObjectWriter(session, incomingAttributes);
        } else {
            writer = new RecordObjectWriter(session, writerFactory, getLogger(), context.getProperty(REGION).getValue(), incomingAttributes);
        }
        ...
        if (totalListCount == 0) {
            getLogger().debug("No new objects in S3 bucket {} to list.", bucket);
        }
    }

    private void listByTrackingTimestamps(ProcessContext context, ProcessSession session, Map<String, String> incomingAttributes) {
        try {
        ...
        final AmazonS3 client = getS3Client(context, incomingAttributes);
        final S3BucketLister bucketLister = getS3BucketLister(context, client, incomingAttributes);
        final String bucket = context.getProperty(BUCKET_WITHOUT_DEFAULT_VALUE).evaluateAttributeExpressions(incomingAttributes).getValue();
        final int batchSize = context.getProperty(BATCH_SIZE).asInteger();
    ...
    private S3BucketLister getS3BucketLister(ProcessContext context, AmazonS3 client, Map<String, String> incomingAttributes) {
        ...
        final String bucket = context.getProperty(BUCKET_WITHOUT_DEFAULT_VALUE).evaluateAttributeExpressions(incomingAttributes).getValue();
        final String delimiter = context.getProperty(DELIMITER).evaluateAttributeExpressions(incomingAttributes).getValue();
        final String prefix = context.getProperty(PREFIX).evaluateAttributeExpressions(incomingAttributes).getValue();
    ...
    public class S3ObjectBucketLister implements S3BucketLister {
        ...
        public VersionListing listVersions() {
            ...
            versionListing.setVersionSummaries(objectListing.getObjectSummaries().stream()
                    .map(CustomListS3.this::objectSummaryToVersionSummary)
    ...
    static class RecordObjectWriter implements S3ObjectWriter {
        ...
        private final Map<String, String> incomingAttributes;
        private RecordSetWriter recordWriter;
        private FlowFile flowFile;

        public RecordObjectWriter(final ProcessSession session, final RecordSetWriterFactory writerFactory, final ComponentLog logger, final String region, final Map<String, String> incomingAttributes) {
        ...
        public void beginListing() throws IOException, SchemaNotFoundException {
            ...
            flowFile = session.putAllAttributes(flowFile, incomingAttributes);
    ...
    static class AttributeObjectWriter implements S3ObjectWriter {
        ...
        public AttributeObjectWriter(final ProcessSession session, final Map<String, String> incomingAttributes) {
            this.session = session;
            this.incomingAttributes = incomingAttributes;
        }
